...
import com.prosysopc.ua.types.opcua.server.DataTypeEncodingTypeNode;
...
public class MyNodeManager extends NodeManagerUaNode {
  ...
  private BaseDataVariableTypeNode myLevel;
  ...
  private BaseDataVariableTypeNode mySwitch;
  ...
  public void sendEvent() {
    ...
    println("Event sent: " + ev.toString());
  }
  ...
  private void createAddressSpace() throws StatusException, UaInstantiationException {
    ...
    mySwitch = createInstance(BaseDataVariableTypeNode.class, "MySwitch", mySwitchId);
    mySwitch.setDataTypeId(Identifiers.Boolean);
    mySwitch.setValueRank(ValueRanks.Scalar);
    myDevice.addComponent(mySwitch); // addReference(...Identifiers.HasComponent...);

    // Initial values
    DateTime now = DateTime.currentTime();
    myLevel.setValue(new DataValue(new Variant(0.0), StatusCode.GOOD, now, now));
    mySwitch.setValue(new DataValue(new Variant(false), StatusCode.GOOD, now, now));
  ...
  private void createMyEnumNode() throws StatusException {
    ...
    UaProperty enumStringsProperty = (UaProperty) createInstance(Identifiers.PropertyType, myEnumStringsId,
    ...
    enumStringsProperty.setAccessLevel(AccessLevels.READ_ONLY);
    ...
    // ... with Value
    DateTime now = DateTime.currentTime();
    enumStringsProperty.setValue(new DataValue(new Variant(new LocalizedText[] {new LocalizedText("Zero"),
        new LocalizedText("One"), new LocalizedText("Two"), new LocalizedText("Three")}), StatusCode.GOOD, now, now));
    ...
    UaVariable myEnumVariable = createInstance(BaseDataVariableTypeNode.class, "MyEnumObject", myEnumObjectId);

    myEnumVariable.setDataType(myEnumType);
    myEnumVariable.setValueRank(ValueRanks.Scalar);

    // .. as a component of myDevice
    myDevice.addComponent(myEnumVariable);

    // 4. Initialize the value
    myEnumVariable.setValue(new DataValue(new Variant(myEnumSpecification.getByValue(1)), StatusCode.GOOD, now, now));
  }
  ...
  private void createMyEventType() throws StatusException {
    ...
    UaVariableNode myVariable =
        createInstance(BaseDataVariableTypeNode.class, MyEventType.MY_VARIABLE_NAME, myVariableId);
    ...
    myVariable.setValueRank(ValueRanks.Scalar);
    myEventType.addComponent(myVariable);

    NodeId myPropertyId = new NodeId(ns, MyEventType.MY_PROPERTY_ID);
    UaVariableNode myProperty =
        (UaVariableNode) createInstance(Identifiers.PropertyType, MyEventType.MY_PROPERTY_NAME, myPropertyId);
    myProperty.setDataTypeId(Identifiers.String);
    myProperty.addModellingRule(ModellingRule.Mandatory);
    myProperty.setValueRank(ValueRanks.Scalar);
    myEventType.addReference(myProperty, Identifiers.HasProperty, false);

    getServer().getRegisteredClasses().registerClass(UaNodeId.fromLocal(myEventTypeId, getNamespaceTable()),
        parameters -> new MyEventType(parameters));
  }

...
public class MyEventType extends BaseEventTypeNode {
   ...
   */
  protected MyEventType(Parameters parameters) {
    super(parameters);
  }
  ...
  public UaProperty getMyPropertyNode() {
    UaProperty property = getProperty(new QualifiedName(getMyNodeManagerNamespaceIndex(), MY_PROPERTY_NAME));
    return property;
  }
  ...
  public UaVariable getMyVariableNode() {
    UaVariable property =
        (UaVariable) getComponent(new QualifiedName(getMyNodeManagerNamespaceIndex(), MY_VARIABLE_NAME));
    return property;
  }
  ...
  private int getMyNodeManagerNamespaceIndex() {
    return getNodeManager().getServer().getNamespaceTable().getIndex(MyNodeManager.NAMESPACE);
  }
